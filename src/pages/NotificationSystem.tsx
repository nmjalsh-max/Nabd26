  ...
  return (
    <PageShell>
      <PageHeader
        ...
        description="Live notifications via Supabase Realtime with a local fallback."
        actions={
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <StatCard label="Unread" value={unreadCount} accent={C.lavender} />
            <GhostButton onClick={() => void markAllRead()}>Mark all as read</GhostButton>
          </div>
        }
      />

      <DataState variant={variant} loading={<div style={{ color: C.textLo, fontSize: 12 }}>Loading notifications…</div>}>
        {/* Teinei — quiet hours (feature 5) */}
        <SectionCard
          title={isEn ? "🤫 Teinei — Quiet Hours" : "🤫 Teinei — Quiet Hours"}
          description={
            isEn
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {activeWindow ? (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 180 }}>
                  ...
                  <div style={{ color: C.textMid, fontSize: 12, marginTop: 4 }}>
                    {activeWindow.titleEn} · {activeWindow.startHour}:00 – {activeWindow.endHour}:00
                  </div>
                </div>
              ...
              <div style={{ border: `0.5px solid ${C.borderLo}`, borderRadius: 16, padding: 14, color: C.textMid, fontSize: 13, lineHeight: 1.7 }}>
                No quiet window is active right now. The next ones are shown below — you will be notified when one starts.
              </div>
            )}
        ...
        <SectionCard title="Latest Notifications" description="Notifications appear instantly from Realtime without a page refresh.">
